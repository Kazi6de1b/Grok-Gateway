...
| 📊 | **Token 用量统计** | 解析 input / output / cached tokens，7 日图表 |
| ≡ | **请求日志** | 账号、模型、状态、耗时、错误码 |
| 🧩 | **每账号模型列表** | 拉取 `/v1/models` 并缓存展示 |
| 🔑 | **可选网关 API Key** | 保护 `/v1/*`；支持 OAuth + API Key 账号 |
...
若在控制台开启了「强制校验网关 API Key」，第三方客户端需带：
...
真实上游鉴权仍由账号池 OAuth / API Key 账号完成。

**模型提示：** 免费 Build 档请用 `/v1/responses` + 账号可用模型（常见 `grok-4.5`），错误模型可能收到上游 `402`。
...
观测数据默认落在 EXE 同目录 `data/requests.jsonl`（请求摘要与日聚合，**不含** OAuth Token）。
